    ...
    <div class="debug_area">
        ...
        <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                <li><a href="#" onclick="saveAs('last', '/tasks')">Tasks</a></li>
...
<script>
        ...
        d3.text("/graph/connectivity")
                ...
                .post(JSON.stringify({nodes: nodes, links: links}), function (error, text) {
                    ...
                    if (response.status != "OK") {
                        $("#description")
                                .html("<strong>Error: </strong>graph contains next unconnected groups:<br> " + response.description);
    ...
    function saveAs(filename, link) {
        ...
        d3.text("/file/" + filename + "/systems/save")
                .header("Content-type", "application/json")
                .post(text, function (error, text) {

                });
        text = '\'' + $('#svg-container').html() + '\'';
        d3.text("/file/" + filename + "/systems/save/image")
                .header("Content-type", "application/json")
                .post(text, function (error, text) {
...
//        var text = '\'' + JSON.stringify({nodes: nodes, links: links}) + '\'';
        d3.text("/file/" + filename + "/systems/load")
                .header("Content-type", "application/json")
